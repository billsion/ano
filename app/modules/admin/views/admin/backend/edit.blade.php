...
<div class="row">
    <div class="col-xs-12">
    {{ Form::open(array('url' => Uri::to('admin/' . $data['id'], false, true), 'method' => 'PUT', 'id' => 'admin-form', 'data-toggle' => 'validator', 'class' => 'form-horizontal')) }}
        <div class="form-group">
            <label class="col-sm-3 control-label no-padding-right">
            {{ Lang::get('admin::admin.label') }}
            </label>
            <div class="col-sm-9">
                <input value="{{ $data['label']; }}" type="text" name="label" class="col-xs-10 col-sm-5" required /><br/>
            </div>
        </div>
        <div class="form-group">
            <label class="col-sm-3 control-label no-padding-right">
            ...
            <div class="col-sm-9">
                <input value="{{ $data['username']; }}" type="text" name="username" class="col-xs-10 col-sm-5" required /><br/>
            </div>
        </div>
        <div class="form-group">
            <label class="col-sm-3 control-label no-padding-right">
                ...
                <ul class="ace-thumbnails clearfix" id="ul-avatar-wrapper">
                    ...
                    <li>
                        <a href="" data-rel="colorbox" class="cboxElement">
                            <img width="150" height="150" src="<?php echo $data['avatar']; ?>" alt="" />
                            <input id="input-hidden-avatar" type="hidden" name="avatar" value="<?php echo $data['avatar']; ?>" />
                        </a>
        ...
        <div class="form-group">
        	...
        	<div class="col-sm-9">
        	    <?php $admin_role_id = array_flip(explode(',', $data['admin_role_id']));?>
        	    <?php foreach($roles as $_role):?>
				    <label class="control-label no-padding-right">
				        <input <?php if(array_key_exists($_role['id'], $admin_role_id)){ echo 'checked="checked"'; }?> value="<?php echo $_role['id']; ?>" name="admin_role_id[]" type="checkbox" class="ace">
	...
	$('#input-file-avatar').uploadify({
		...
		'onUploadSuccess': function(file, data, response) {
		  ...
		  $('#ul-avatar-wrapper').children().remove();
			...
			$('#ul-avatar-wrapper').append('<li><a href="" data-rel="colorbox" class="cboxElement"><img width="150" height="150" src="'+data+'" alt="" /><input id="input-hidden-avatar" type="hidden" name="avatar" value="'+data+'" /></a><div class="tools tools-bottom"><a href="javascript:;"><i class="ace-icon fa fa-times red"></i></a></div></li>');
